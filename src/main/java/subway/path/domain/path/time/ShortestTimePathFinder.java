...
public class ShortestTimePathFinder {
    ...
    ShortestTimePathFinder(List<GraphPath<Station, SubwayGraphEdge>> graphPaths) {
        requireNotEmpty(graphPaths);

        this.graphPaths = graphPaths;
    }

    private void requireNotEmpty(List<GraphPath<Station, SubwayGraphEdge>> graphPaths) {
    ...
    public PathAndArrival getPath(LocalDateTime departureTime) {
        GraphPath<Station, SubwayGraphEdge> shortestPath = getShortestArrivalTimePath(departureTime);

        return new PathAndArrival(shortestPath, getArrivalTime(shortestPath, departureTime));
    }

    private GraphPath<Station, SubwayGraphEdge> getShortestArrivalTimePath(LocalDateTime departureTime) {
    ...
    private Comparator<GraphPath<Station, SubwayGraphEdge>> arrivalTimeComparator(LocalDateTime departureTime) {
        return Comparator.comparing(path -> getArrivalTime(path, departureTime));
    }


    private LocalDateTime getArrivalTime(GraphPath<Station, SubwayGraphEdge> path, LocalDateTime departureTime) {
        List<SubwayGraphEdge> sections = path.getEdgeList();
        List<Station> stations = path.getVertexList();

        LocalDateTime arrivalTime = getBoardingTime(departureTime, sections, stations);
        for (int i = 0; i < sections.size(); i++) {
            SubwayGraphEdge section = sections.get(i);
    ...
    private LocalDateTime getBoardingTime(LocalDateTime departureTime, List<SubwayGraphEdge> edges, List<Station> stations) {
