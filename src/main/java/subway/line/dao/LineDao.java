...
public class LineDao {
    public static final String FIND_LINE_BY_LINE_ID = "select L.id as line_id, L.name as line_name, L.color as line_color, L.extra_fare as line_extra_fare, " +
            ...
            "WHERE L.id = ?";
    public static final String FIND_ALL_LINE = "select L.id as line_id, L.name as line_name, L.color as line_color, L.extra_fare as line_extra_fare, " +
            "S.id as section_id, S.distance as section_distance, " +
            "UST.id as up_station_id, UST.name as up_station_name, " +
            "DST.id as down_station_id, DST.name as down_station_name " +
            "from LINE L \n" +
            ...
            "left outer join STATION UST on S.up_station_id = UST.id " +
            "left outer join STATION DST on S.down_station_id = DST.id ";
    public static final String UPDATE_LINE = "update LINE set name = ?, color = ? where id = ?";
    public static final String DELETE_LINE = "delete from Line where id = ?";
    ...
    public Line findById(Long id) {
        List<Map<String, Object>> result = jdbcTemplate.queryForList(FIND_LINE_BY_LINE_ID, new Object[]{id});
        return mapLine(result);
    }

    public void update(Line newLine) {
        jdbcTemplate.update(UPDATE_LINE, new Object[]{newLine.getName(), newLine.getColor(), newLine.getId()});
    }

    public List<Line> findAll() {
        List<Map<String, Object>> result = jdbcTemplate.queryForList(FIND_ALL_LINE);
    ...
    public void deleteById(Long id) {
        jdbcTemplate.update(DELETE_LINE, id);
    }
}
