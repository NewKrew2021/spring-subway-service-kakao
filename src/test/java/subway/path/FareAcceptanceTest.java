...
public class FareAcceptanceTest extends AcceptanceTest {
  ...
  public static final String 유아_이메일 = "infant@email.com";
  ...
  private TokenResponse 유아;

  /**
   * 교대역  --- *2호선*(28) ---   강남역   --- *2호선*(38) ---   잠실역 |                        | *3호선*(5)
   * *신분당선*(20) |                        | 남부터미널역  --- *3호선*(3) ---   양재역
   */
  @BeforeEach
  public void setUp() {
    super.setUp();
  ...
  void infantFare() {
    // when
    ExtractableResponse<Response> response = 토큰_포함_최단_경로_검색_요청(유아, 남부터미널역, 강남역);

    // then
    최단_경로_요금_조회됨(response, 0);
  }
  ...
  private ExtractableResponse<Response> 최단_경로_검색_요청(StationResponse sourceStation,
      StationResponse targetStation) {
    return RestAssured
        .given().log().all()
        .accept(MediaType.APPLICATION_JSON_VALUE)
        .when().get(String
            .format("/paths?source=%d&target=%d", sourceStation.getId(), targetStation.getId()))
        .then().log().all()
        .extract();
  }

  private ExtractableResponse<Response> 토큰_포함_최단_경로_검색_요청(TokenResponse tokenResponse,
